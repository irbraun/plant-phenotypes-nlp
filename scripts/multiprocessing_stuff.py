...
from oats.graphs.pairwise import subset_edgelist_with_ids
...
dataset = Dataset()
dataset.add_data(pd.read_csv("../data/reshaped_files/all_pppn_gene_textlong.csv", lineterminator="\n"))
...
dataset.describe()
dataset.filter_random_k(k=1000, seed=78263)
...
# Get mappings between object IDs and lists of pathways IDs.
id_to_pathway_ids = merge_list_dicts(groupings_kegg.get_forward_dict(genes), 
...
df_train, df_test = train_test_split(df_subset, test_size=0.95)
...
graphs = [IndexedGraph(df=df, value=name) for name in names]
