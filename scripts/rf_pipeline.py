...
if __name__ == "__main__":
	...
	dataset.randomly_subsample_dataset(n=80, seed=78263)
	...
	# Use parallel processing the build all the matrices.
	...
	# Look at how long it took to build each matrix.
	print("Durations of generating each pairwise matrix (seconds)")
	durations = [result[1] for result in results]
	for (name,duration) in zip(names,durations):
		print("{:12} {}".format(name, duration))
	print("MP time was {0:.4f} of the single core time.".format(total_time_mp/sum(durations)))
